...
def processing(img):
    ...
    if ceil_distance < 100:
        ...
        result = analize_space(
            obs_all_mask, ceil_position_x_prom, ceil_position_y_prom, canvas_img=img,
        )
        ...
        try:
            ...
            print(results)
            print(
                'Cantidad de datos alcanzados (%d) %ss result: %d' %
                (len(results), time.time() - results_last_time, result),
            )
            results_last_time = time.time()
        except Exception as e:
            print(e, results)
