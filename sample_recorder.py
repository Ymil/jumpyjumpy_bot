...
def analize_space(img, ceil_x, ceil_y, delta_y=15, delta_x=110, steep_x=3, canvas_img=None):
    ...
    cv.line(
        canvas_img, (ceil_x, ceil_y+delta_y),
        (ceil_x-delta_x, ceil_y-10), (0, 0, 255),
    )
    cv.line(
        canvas_img, (ceil_x, ceil_y+delta_y),
        (ceil_x+delta_x, ceil_y-10), (0, 0, 255),
...
class recorder():
    ...
    def processing_img(self, img, key):
        self.__lock.acquire()
        print(f'Capture {self.__capture_counter}')
        ...
        ceil_space = analize_space(
            obs_all_mask,  self._ceil['x'],  self._ceil['y'], canvas_img=img, steep_x=1, delta_y=20,
        )
        air_space = analize_space(
            obs_all_mask,  self._ceil['x'], self._ceil['y']-75, canvas_img=img, steep_x=1, delta_y=0,
        )
        next_ceil_space = analize_space(
            obs_all_mask,  self._ceil['x'], self._ceil['y']+150, canvas_img=img, steep_x=1, delta_y=25,
        )
    ...
    def processing(self, key):
        ...
        thread = threading.Thread(
            target=self.processing_img, args=(self.__frame, key),
        )
        thread.start()
    ...
    def on_release(self, key):
        if key == keyboard.Key.left or key == keyboard.Key.right or key == keyboard.Key.space:
            ...
            print('End Capture')
            filename = f'{folder_dest}/{time.time()}-{str(key)}d-{time_delta}s.JPG'
            thread = threading.Thread(
                target=self.write, args=(filename, self.__frame),
            )
            thread.start()
            self.__press = False
            # self.__lock.release()
    ...
    def save(self):
        df = pd.DataFrame(
            self._data, columns=[
            'key', 'ceil_distance', 'colision', 'ceil_space', 'air_space', 'next_level_space',
            ],
        )
