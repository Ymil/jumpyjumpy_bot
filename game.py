...
from main import analize_space
from main import ball_deteccion
from main import ceil_deteccion
from main import measure_distance_to_ceil_and_detted_colission
from main import obstacle_deteccion
...
def analize_ceil(
    img, ceil_x, ceil_y, second_width=45,
    ...
    print(f'Processing 47 time {time.time() - time_start_47}s')
    cy_ = ceil_y+delta_y
    left_list = []
    right_list = []
    ...
    print(f'Processing 61 time {time.time() - time_start_61}s')
    ...
    print(f'Processing 78 time {time.time() - time_start_78}s')
...
class recorder():
    ...
    def measure_ceil_distance(self, repeat=100):
        ...
        while True:
            ...
            for _ in range(repeat):
                ...
                ceil_cx, ceil_cy, ceil_distance, colision = measure_distance_to_ceil_and_detted_colission(
                    obs_all_mask, cx, cy,
                )
                if ceil_cx == 0:
                    continue
            ...
            img = self.get_image()
            cv.circle(
                img, (self._ceil['x'], self._ceil['y']), 2, (0, 0, 255), 10,
            )
            print("Press 'n' for recalibrate")
            print(
                'Ceil position {} {}'.format(
                    self._ceil['x'], self._ceil['y'],
                ),
            )
            cv.imshow('', img)
        ...
        while True:
            ...
            try:
                next_ceil_y = int(
                    input(
                        f'Insert delta px to delta ceil [{self.next_ceil_y}]> ',
                    ),
                )
            ...
            print(
                'Next Ceil position {} {}'.format(
                    self._ceil['x'], self._ceil['y']+self.next_ceil_y,
                ),
            )
            cv.imshow('', img)
            if cv.waitKey(0) == ord('y'):
    ...
    def ceil_thread(self, mask, img):
        ceil_space = analize_ceil(
            mask,  self._ceil['x'],  self._ceil['y'], canvas_img=img, steep_x=1, delta_y=20, draw=True,
        )
        self.data['ceil_space'] = ceil_space

    def next_ceil_space_thread(self, mask, img):
        next_ceil_space = analize_ceil(
            mask,  self._ceil['x'], self._ceil['y']+self.next_ceil_y, canvas_img=img, steep_x=1, delta_x=90, second_width=65, draw=True,
        )
        self.data['next_ceil_space'] = next_ceil_space

    def air_space_thread(self, mask, img):
        air_space = analize_space(
            mask,  self._ceil['x'], self._ceil['y']-100, canvas_img=img, width=310, draw=True,
        )
    ...
    def processing(self, img):
        ...
        print(
            'Capture {} len {}'.format(
                self.__capture_counter, len(self._data),
            ),
        )
        ...
        ceil_thread = threading.Thread(
            target=self.ceil_thread, args=(obs_all_mask, img),
        )
        next_ceil_space_thread = threading.Thread(
            target=self.next_ceil_space_thread, args=(obs_all_mask, img),
        )
        air_space_thread = threading.Thread(
            target=self.air_space_thread, args=(obs_all_mask, img),
        )
        ceil_thread.start()
        ...
        if not stats.mode(self.data['ceil_space']) == 1 and not stats.mode(self.data['next_ceil_space']) == 1:
            data = np.array(
                [
                    self.data['air_space'], self.data['ceil_space'],
                    self.data['next_ceil_space'],
                ],
            )
...
if __name__ == '__main__':
    while True:
        ...
        start = time.time()
        frame = r.get_image()
        print(f'Capture {time.time() - start}s')
        start = time.time()
        img = r.processing(frame)
        cv.imshow('', img)
        print(f'Processing {time.time() - start}s')
